...
再生・音量・ミュートは **Twitchプレイヤー本体のネイティブコントロール** を使用します。
...
- 上部の **先読み** プルダウンで、次の配信を裏で何本先読みしておくか（なし〜3本）を設定できます。
  本数を増やすと配信切り替えは速くなりますが、裏で先読み再生する分の通信量・負荷が増えます。
  一時停止中は先読みも止まり、再生を再開すると先読みも再開します。
...
│   │   ├── ZapMode/usePlayerPool.js Twitchプレイヤーのプール管理（先読み・一時停止連動）
